	...
	public function delete_checkout_cookies() {
		...
		if ( _is_wcf_thankyou_type() ) {
			...
			if ( isset( $_COOKIE[ CARTFLOWS_ACTIVE_CHECKOUT ] ) ) {
				delete_transient( 'wcf_user_' . $user_key . '_checkout_' . sanitize_text_field( wp_unslash( $_COOKIE[ CARTFLOWS_ACTIVE_CHECKOUT ] ) ) );
				unset( $_COOKIE[ CARTFLOWS_ACTIVE_CHECKOUT ] );
				setcookie( CARTFLOWS_ACTIVE_CHECKOUT, '', time() - 3600 );
			}
		}
	}
